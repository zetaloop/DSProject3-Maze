...
class MazeApp:
    def __init__(self, root):
        ...
        self.maze_types = {
            "空白": (
                "generate_empty_maze",
                "无任何障碍的地形，适合观察算法的基本搜索模式",
            ),
            "障碍": (
                "generate_random_maze",
                "随机分布的墙体，模拟自然环境中的随机障碍",
            ),
            "迷宫": (
        ...
        self.algorithm_descriptions = {
            "深度优先搜索 (DFS)": "优先探索深度方向，内存占用小但不保证最短路径",
            "广度优先搜索 (BFS)": "逐层扩展搜索范围，在无权图中保证最短路径",
            "双向广度优先搜索": "同时从起点和终点搜索，在大型迷宫中效率更高",
            "贪心搜索": "基于启发式估计选择路径，速度快但不保证最短路径",
            "A*搜索": "结合实际代价和启发式估计，通常能找到最优路径",
        }
